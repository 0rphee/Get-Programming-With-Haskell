...
showTVPair :: (Show a) => Int -> Maybe a -> Int -> String
showTVPair time (Just value) nDigits = mconcat [formattedTime,"|",show value,"\n"]
    where formattedTime = formatTime (show time) nDigits
showTVPair time Nothing nDigits = mconcat [formattedTime,"|NA\n"]
    where formattedTime = formatTime (show time) nDigits

formatTime :: [Char] -> Int -> [Char]
formatTime xs nDigits
    | xsLen >= nDigits = xs
    | otherwise = formatTime (' ':xs) nDigits
    where xsLen = length xs


instance Show a => Show (TS a) where
    show (TS times values) = mconcat rows
        where rows = zipWith (\x y -> showTVPair x y nDigits) times values
              maxVal = maximum times
              nDigits = length (show maxVal)
